const Game = {
  ...
  update() {
    ...
    const headY = player.y - 28 - player.nod * 12;
    ...
    if (ball.vy > 0 && dist < hitR) {
      ...
      this.score++;
      this.streak++;
      this.addPopup(ball.x, ball.y - 24, '+1', '#FFD700', false);
      if (this.streak % 5 === 0) {
        this.addPopup(player.x, headY - 40, `🔥${this.streak}!`, '#FF6B35', true);
      }
      this.shake = 4;
  ...
  die(reason) {
    ...
    if (window.ArcadeAnalytics) {
      ArcadeAnalytics.track('game_over', { game: 'kopfball-karo', score: this.score, reason });
    }
  ...
  getMedal(score) {
    if (score >= 50) return { icon: '🥇', name: 'Karo-Legende' };
    if (score >= 30) return { icon: '🥈', name: 'Stadion-Star' };
    if (score >= CONFIG.CHALLENGE_SCORE) return { icon: '🥉', name: 'Kopfball-Pro' };
    return null;
  },
