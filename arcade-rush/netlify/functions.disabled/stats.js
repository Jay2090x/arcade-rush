...
var require_analytics_store = __commonJS({
  "scripts/functions-src/_analytics-store.js"(exports2, module2) {
    ...
    function metaToEvent2(counterKey, meta = {}) {
      const ts = Date.now();
    ...
    module2.exports = {
      ...
      metaToEvent: metaToEvent2
    };
  }
});

// scripts/functions-src/stats.src.js
...
async function mirrorToAnalytics(context, event, counterKey, meta) {
  const ev = metaToEvent(counterKey, meta);
  if (!ev) return;
  try {
    const store = openStore(context, event);
    const report = await loadReport(store);
    const geo = context?.geo?.country?.code || event.headers?.["x-country"];
    if (geo && !ev.country) ev.country = geo;
    ingestEvent(report, ev);
    await saveReport(store, report);
  } catch (err) {
    console.error("stats mirror error:", err.message);
  }
}
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      await mirrorToAnalytics(context, event, key, meta);
    }
    return {
      statusCode: 200,
      headers: CORS,
